# pages/break_schedule.py  —  Screen 38: Break Schedule + Screen 39: Log Break
# The signature feature: a Gantt-style visual timeline per staff member,
# showing shifts (blue) with break notches cut out (white/coloured).

import streamlit as st
from datetime import datetime, date, time as _time, timedelta

from utils.break_queries import (
    fetch_breaks_today, create_break, update_break_schedule,
...
def render():
    ...
    if st.session_state.get("show_log_break"):
        st.markdown("---")
        _render_log_break_form(centre_id, all_shifts, rooms, attendance, now)
        st.markdown("---")
    ...
    if missed > 0:
        st.warning(
            ...
            f"Review the schedule below and mark breaks as taken or rescheduled."
        )
    if in_progress > 0:
        st.info(
            f"☕ **{in_progress} staff member(s) currently on break** — "
            f"they are excluded from ratio calculations until they return."
        )

    st.markdown("---")

    # ── Ratio impact warnings ─────────────────────────────────────────
    warnings = _check_ratio_impact(all_breaks, all_shifts, rooms, attendance, now)
    if warnings:
        with st.expander(f"⚠️ {len(warnings)} ratio impact warning(s)", expanded=True):
            for w in warnings:
                st.warning(w)
    ...
    st.caption(
        ...
        "White = scheduled break, green = completed, red = missed, yellow = in progress."
    )

    # Group breaks by user_id for fast lookup
    ...
    if not all_shifts:
        st.info("No shifts rostered today. Breaks can only be scheduled for staff with an active shift.")
    else:
        ...
        for shift in sorted(all_shifts, key=lambda s: (
            ...
            _render_gantt_row(shift, u_breaks, centre_id, now)

    # ── Break status table ────────────────────────────────────────────
    st.markdown("---")
    st.markdown("### 📋 Break Status Table")
    _render_break_table(all_breaks, all_shifts, now, centre_id)
...
def _render_timeline_header():
    """Render hour labels above the Gantt chart."""
    hours = list(range(DAY_START, DAY_END + 1))
    # Staff name column width = 18% of total, timeline = 82%
    name_w = 18
    tl_w   = 82
    ...
    for h in hours:
        ...
        label = f"{h}:00" if h < 12 else f"{h-12 if h > 12 else 12}pm" if h >= 12 else f"{h}am"
        hour_labels_html += (
...
def _render_gantt_row(shift: dict, breaks: list, centre_id: str, now: str):
    """Render one Gantt row: name label + visual bar + quick action buttons."""
    ...
    entitlement = calc_break_entitlement(shift_mins)

    # Build Gantt bar segments
    segments = build_gantt_bars(
        sstart + ":00", send + ":00", breaks, DAY_START, DAY_END
    )

    # Current-time marker on this row
    now_h, now_m = datetime.now().hour, datetime.now().minute
    now_pct = ((now_h + now_m / 60 - DAY_START) / (DAY_END - DAY_START)) * 100
    ...
    # Room colour dot at end of name
    room_dot = (
    ...
    # Render row HTML + action buttons inline
    ...
    with col_btn:
        if st.button(
            "＋",
            key=f"add_break_{shift_id}",
            help=f"Schedule a break for {sname}",
...
def _render_break_table(breaks: list, shifts: list, now: str, centre_id: str):
    """Detailed table of all breaks with inline actions."""
    # Build a shift lookup for context
    shift_map = {s["id"]: s for s in shifts}
    user_shift_map: dict[str, dict] = {}
    for s in shifts:
        uid = s.get("user_id", "")
        if uid not in user_shift_map:
            user_shift_map[uid] = s

    if not breaks:
        st.info(
            "No breaks scheduled today. "
            "Click **➕ Schedule Break** or the **＋** button next to a staff member."
        )
        # Show staff without breaks as a reminder
        unscheduled = [
            s for s in shifts
        ...
        if unscheduled:
            ...
            for s in unscheduled:
                ...
                sstart = (s.get("start_time") or "")[:5]
                send   = (s.get("end_time")   or "")[:5]
                dur    = shift_duration_minutes(sstart, send)
                ent    = calc_break_entitlement(dur)
                if ent["total_min"] > 0:
                    st.markdown(
    ...
    # Render each break record
    for b in sorted(breaks, key=lambda x: (
        x.get("planned_start_time") or ""
    )):
        _render_break_row(b, shift_map, now, centre_id)


def _render_break_row(b: dict, shift_map: dict, now: str, centre_id: str):
    """Render one break record as an expandable row with actions."""
    bid        = b.get("id", "")
    uid        = b.get("user_id", "")
    u          = b.get("users") or {}
    sname      = f"{u.get('first_name','')} {u.get('last_name','')}".strip() or "Unknown"
    ...
    # Room from shift
    shift      = shift_map.get(b.get("roster_shifts", {}).get("id") if isinstance(b.get("roster_shifts"), dict) else "", {})
    room_name  = ""
    room_colour = "#3498DB"
    ...
    with st.expander(
        ...
        expanded=(live_status in ("in_progress", "missed")),
    ):
        # Top row
        ...
        if live_status not in ("completed",):
            if ab2.button("✏️ Edit", key=f"edit_b_{bid}", use_container_width=True):
                st.session_state[f"edit_break_{bid}"] = not st.session_state.get(f"edit_break_{bid}", False)
                st.rerun()

        # Mark missed
        if live_status in ("scheduled", "in_progress") and ab3.button(
            "⚠️ Mark Missed", key=f"miss_{bid}", use_container_width=True
        ):
            try:
                mark_break_missed(bid)
                toast_warn(f"Break marked as missed.")
                st.rerun()
            except Exception as e:
                toast_error(str(e))
...
def _render_mark_taken_form(bid: str, b: dict):
    """Inline form to record actual break times."""
...
def _render_edit_break_form(bid: str, b: dict):
    """Inline form to edit a scheduled break's times."""
    ...
    with st.form(f"edit_form_{bid}"):
        ...
        new_type  = ec1.selectbox("Break type", options=type_opts,
                                   index=type_idx,
...
# ─────────────────────────────────────────────────────────────────────────────
# LOG BREAK FORM (Screen 39)
# ─────────────────────────────────────────────────────────────────────────────

def _render_log_break_form(
    centre_id: str,
    all_shifts: list,
    rooms: list,
    attendance: list,
    ...
    """Full form for scheduling a new break, with ratio impact preview."""
    st.markdown("### Schedule a Break")

    # Build staff options from today's shifts
    ...
    if not staff_opts:
        st.warning("No staff rostered today to schedule breaks for.")
    ...
    with st.form("log_break_form", clear_on_submit=False):
        ...
        # Pre-fill if coming from the ＋ button on a specific row
        prefill_uid = st.session_state.get("prefill_user_id", "")
        uid_list    = list(staff_opts.keys())
        uid_default = uid_list.index(prefill_uid) if prefill_uid in uid_list else 0
        ...
        # Show entitlement hint based on selected staff's shift
        ...
        if ss and se:
            ...
            ent      = calc_break_entitlement(dur_mins)
        ...
        # Actual times (optional — for recording retrospectively)
        ...
        lb_notes = st.text_input(
            "Notes",
            placeholder="e.g. Covering Nursery until back — swap arranged",
            key="lb_notes",
        )
        ...
        if room_id:
            ...
            if room_cfg:
                # Simulate removing this staff member during the break window
                n_children = sum(
                ...
                # Remove one staff for the break
                staff_during_break = max(0, total_staff - 1)
                ...
                st.markdown(
                    ...
                    + (f'. Consider moving the break.' if impact["status"] in ("breach","warning") else '. Safe to proceed.')
                    + f'</div>',
                    unsafe_allow_html=True,
                )
        ...
        submitted = sc1.form_submit_button(
            "💾 Save Break", type="primary", use_container_width=True
        )
...
def _check_ratio_impact(
    breaks: list, shifts: list, rooms: list, attendance: list, now: str
) -> list[str]:
    """
    Check if any scheduled break during the current/next hour would
    cause a ratio issue in their room.
    """
    warnings = []
    ...
    for b in breaks:
        ...
        # Find their shift
        shift = next(
            (s for s in shifts if s.get("user_id") == uid), None
        )
        if not shift:
            continue
        ...
        # Total staff currently in this room
        total_in_room = sum(
            1 for s in shifts
            if s.get("room_id") == room_id
