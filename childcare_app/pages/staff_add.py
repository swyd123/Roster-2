...
def render():
    ...
    with tc:
        ...
        st.markdown('<p class="page-sub">Fill in the details below. Fields marked * are required.</p>',
    ...
    if values:
        with st.spinner("Creating staff member…"):
            try:
                ...
                if profile:
                    toast_success(f"{values['first_name']} {values['last_name']} has been added.")
                    time.sleep(0.8)
                ...
                if "duplicate" in err.lower() or "unique" in err.lower():
                    toast_error(f"A staff member with email **{values['email']}** already exists.")
                else:
